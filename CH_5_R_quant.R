...
### 코스닥 업종분류 데이터 다운로드
down_sector_KQ = POST(down_url, query = list(code = otp), add_headers(referer = gen_otp_url)) %>%
  read_html(encoding = 'EUC-KR') %>%
  html_text() %>%
  read_csv()
...
KOR_ticker = merge(down_sector, down_ind, by=intersect(names(down_sector), names(down_ind)), all = F)
...
url = paste0('http://www.wiseindex.com/Index/GetIndexComponets', '?ceil_yn=0&dt=20210226&sec_cd=G10')
data = fromJSON(url)
...
for (i in sector_code){
  url = paste0('http://www.wiseindex.com/Index/GetIndexComponets', '?ceil_yn=0&dt=20210226&sec_cd=', i)
  data = fromJSON(url)
  data = data$list
